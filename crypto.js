...
var prefix = Neptuneconfig.prefix;
...
client.elevation = message => {
  ...
  if (message.author.id === Neptuneconfig.sahip) permlvl = 4;
  return permlvl;
};
...
client.login(Neptuneconfig.token)
...
.setDescription('<a:Neptunesiyahhalka:769956810785947669> Aleyküm selam. Hoş geldin!')
.setTimestamp()
.setFooter('Neptune')
.setColor(0x36393E)
...
client.on("messageUpdate", async (old, nev) => {
 ...
 const embed = new Discord.MessageEmbed() .setColor(0x36393F) .setDescription(`<a:Neptunered:770187639801774080> ${nev.author} , **Mesajını editleyerek küfür etmeye çalıştı!**`)
            .addField("Mesajı:",nev)
        
            nev.delete();
            const embeds = new Discord.MessageEmbed() .setColor(0x36393F) .setDescription(`<a:Neptunered:770187639801774080> ${nev.author} , **Mesajı editleyerek küfür etmene izin veremem!**`) 
...
client.on("message", async msg => {
    ...
    let i = await db.fetch(`küfür.${msg.member.guild.id}.durum`);
     ...
     msg.delete({timeout:750});
                    const embeds = new Discord.MessageEmbed() .setColor(0x36393F) .setDescription(`<a:Neptunered:770187639801774080> <@${msg.author.id}> , **Bu sunucuda küfür yasak!**`)
      msg.channel.send(embeds).then(msg => msg.delete({timeout: 5000}));
                const embed = new Discord.MessageEmbed() .setColor(0x36393F) .setDescription(`<a:Neptunered:770187639801774080> ${msg.author} , **Küfür etmeye çalıştı!**`) .addField("Mesajı:",msg)
               client.channels.cache.get(y).send(embed)
                  }              
                } catch(err) {
                  console.log(err);
...
client.on("guildMemberAdd", member => {
  ...
  if (ayyy < 1) {
    kontrol = "**Şüpheli** <a:Neptunered:770187639801774080>";
  }
  if (ayyy > 1) {
    kontrol = "**Güvenilir** <a:Neptuneonay:770187690402250772>";
  }
 ...
 //
  .setDescription(`<a:Neptunewelcm1:770584763336622090><a:Neptunewelcm2:770584759340105738> **Hoş geldin!** ${
        member.user
      }, seninle beraber **${
        guild.memberCount
      }** kişi olduk! \n <a:Neptuneelms:770585154204729356> Kaydının yapılması için **isim** ve **yaş** yazman gerek. \n <a:Neptuneyuklnyr:770585554483806248> Hesap kuruluş tarihi: **${moment(
        user.createdAt
      ).format("DD")} ${aylar[moment(user.createdAt).format("MM")]} ${moment(
        user.createdAt
      ).format(
        "YYYY HH:mm:ss"
       )}** \n <a:Neptuneyklnyr2:770585976518475777> Bu vatandaş: ${kontrol} \n <a:Neptunesagok:770586298771701810> <@&${kayıtçı}> rolündeki yetkililer sizinle ilgilenecektir.`);
  //
  client.channels.cache.get(kanal).send(embed);
...
if(Date.now() < maxTime) {
  ...
  .setColor(0x36393F)
  .setDescription(`<a:Neptunered:770187639801774080> <@${message.author.id}> , **Bu sunucuda spam yapmak yasak!**`)
...
if (nev.member.hasPermission("BAN_MEMBERS")) return ;
 //if (ayarlar.gelistiriciler.includes(nev.author.id)) return ;
const embed = new Discord.MessageEmbed() .setColor(0x36393F) .setDescription(`<a:Neptunered:770187639801774080> ${nev.author} , **Mesajını editleyerek reklam yapmaya çalıştı!**`)
      .addField("Mesajı:",nev)
  
      nev.delete();
      const embeds = new Discord.MessageEmbed() .setColor(0x36393F) .setDescription(`<a:Neptunered:770187639801774080> ${nev.author} , **Mesajı editleyerek reklam yapamana izin veremem!**`) 
    client.channels.cache.get(y).send(embed)
      nev.channel.send(embeds).then(msg => msg.delete({timeout:5000}));
    ...
    if (i) {
        if (reklam.some(word => msg.content.toLowerCase().includes(word))) {
          try {
           ...
           //  if (!ayarlar.gelistiriciler.includes(msg.author.id)) return ;
msg.delete({timeout:750});
              const embeds = new Discord.MessageEmbed() .setColor(0x36393F) .setDescription(`<a:Neptunered:770187639801774080> <@${msg.author.id}> , **Bu sunucuda reklam yapmak yasak!**`)
msg.channel.send(embeds).then(msg => msg.delete({timeout: 5000}));
          const embed = new Discord.MessageEmbed() .setColor(0x36393F) .setDescription(`<a:Neptunered:770187639801774080> ${msg.author} , **Reklam yapmaya çalıştı!**`) .addField("Mesajı:",msg)
         client.channels.cache.get(y).send(embed)
            }              
          } catch(err) {
            console.log(err);
...
const ayarlar = require('./Neptuneconfig.json');
...
client.on("guildMemberAdd", (member) => {
    ...
    guild.fetchInvites().then(invites => {
        ...
        if(channel){
          ...
          .setFooter("Neptune")
          .setTimestamp()
          .setDescription(`<a:Neptuneonay:770187690402250772> ${member} adlı kişi sunucuya katıldı. Davet eden şahıs: **${invite.inviter.tag}** (**${total + bonus}** davet!)`)
    ...
    db.add(`invites.${data.inviter}.leave`, 1);
     if(channel){
        let user = client.users.cache.get(data.inviter)
          ...
          .setFooter("Neptune")
          .setTimestamp()
          .setDescription(`<a:Neptunered:770187639801774080> ${member.user.tag} sunucudan ayrıldı. Şahsı davet eden: **${user.tag}** (**${Number(total) + Number(bonus)}** davet!)`)
  ...
  if (db.has(`üyelikk_${msg.author.id}`) == true) {
    if (dakdest !== null && timeout - (Date.now() - dakdest) > 0) {
      ...
      if(msg.content.size > 32){
        ...
        .setAuthor(`Neptune`,`${msg.author.avatarURL() || msg.author.displayAvatarURL()}`)
...
if(mesaj.includes('Neptune')) message.react('735884191547392100');
})
...
if(!link.startsWith('https://')) return message.channel.send('<a:Neptunered:770187639801774080> Bu bir link değil.')
  if(links == null) await wiodb.set("uptime",[])
let map = []
  var x = links
for(var i in x) {
  if(x[i].url.includes(link)) return message.channel.send("<a:Neptunered:770187639801774080> Link sistemde zatem bulunmakta.")
   
}
    message.channel.send("<a:Neptuneonay:770187690402250772> Link sisteme başarıyla eklenmiştir.");
...
.setTimestamp()
.setFooter(`Neptune`)
.setDescription( "<a:Neptuneonay:770187690402250772> **" +
