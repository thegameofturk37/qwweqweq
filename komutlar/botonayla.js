 ...
 let westrabotlistyetkilisi = db.fetch(`westrabotlistyetkilirol_${message.guild.id}`)
  ...
  message.delete()
   const westrabumbeyyyy = new Discord.MessageEmbed()
    ...
    .setFooter(`Neptune`)
    .setTimestamp()
    .setDescription(`<a:Neptuneonay:770187690402250772> <@${sahip}> adlı kişinin <@${botisim}> adlı botu onaylandı. Onaylayan yetkili: ${message.author}`)
		client.channels.cache.get(log).send(westrabumbeyyyy);
		message.channel.send(`<a:Neptuneonay:770187690402250772> Botu onayladınız.`).then(x => x.delete({timeout: 3000}))
};
