...
let westralogkanal = await db.fetch(`westralogkanal_${message.guild.id}`, kanal3.id);
  ...
  if (message.channel.id !== kanald) return message.channel.send(`<a:Neptunered:770187639801774080> Bu komutu sadece <#${kanald}> kanalında kullanabilirsin.`).then(x => x.delete({timeout: 3000}))
	if (message.channel.id == kanald) {
  if (!botid) return message.channel.send(`<a:Neptunered:770187639801774080> Botunun ID'sini yazmalısın.`).then(x => x.delete({timeout: 3000}))
  if (!prefix) return message.channel.send(`<a:Neptunered:770187639801774080> Botunun prefixini yazmalısın.`).then(x => x.delete({timeout: 3000}))
  if (!onaylımı) return message.channel.send(`<a:Neptunered:770187639801774080> Botunun DBL onaylı olup olmadığını yazmalısın.`).then(x => x.delete({timeout: 3000}))
  message.delete()
  ...
  client.channels.cache.get(basvuru).send(embed)
    ...
    .setFooter(`Neptune`)
    .setTimestamp()
    .setDescription(`<a:Neptuneyuklnyr:770585554483806248> ${message.author} adlı kullanıcının <@${botid}> adlı botu sıraya eklendi. Botu onaylanmayı bekliyor.`)
  client.channels.cache.get(log).send(westrabumbeyyyy)
  message.channel.send(`<a:Neptuneonay:770187690402250772> Bot ekleme isteğiniz alındı.`).then(x => x.delete({timeout: 3000}))
  }
};
