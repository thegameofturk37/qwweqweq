  ...
  if (kontrol == "agayokaga") {
    ...
    if (!kanal) {
      const embed = new Discord.MessageEmbed()
        ...
        .setDescription(`<a:Neptunered:770187639801774080> Rol koruma sistemi zaten ayarlanmamış!`);
    ...
    const embed = new Discord.MessageEmbed()
      .setColor(0x36393F)
      .setFooter(client.user.username, client.user.avatarURL())
      .setDescription(`<a:Neptunered:770187639801774080> Rol koruma sistemi sıfırlandı!`);
    message.channel.send(embed);
    return;
  } else {
    let kanal = await db.fetch(`rolk_${message.guild.id}`)
