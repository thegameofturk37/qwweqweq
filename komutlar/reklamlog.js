...
.setDescription(`<a:Neptunered:770187639801774080> **HATA** <a:Neptunered:770187639801774080> - Bu sunucuda yetkili değilsin.`)
    ...
    const member = new Discord.MessageEmbed()
     .setColor(0x36393F)
.setDescription(`<a:Neptunered:770187639801774080> **HATA** <a:Neptunered:770187639801774080> - Bir kanal etiketle.`)
      if(reklam) {
        let kanal = message.mentions.channels.first()
        if(!kanal) return message.channel.send(member)
      db.set(`reklam.${message.guild.id}.kanal`,kanal.id)
      message.channel.send(`<a:Neptuneonay:770187690402250772> **Başarılı ile reklam log kanalı ayarlandı.** `).then(l => {
      l.delete({timeout: 5000})
    })
    }else{
     message.channel.send(`<a:Neptunered:770187639801774080> **Reklam engel açık değil.**`).then(l => {
      l.delete({timeout: 5000})
    })
    }
    }
